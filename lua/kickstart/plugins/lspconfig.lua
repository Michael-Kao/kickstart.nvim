      ...
      local function clangd_query_drivers()
        ...
        for _, compiler in ipairs { vim.env.CXX, vim.env.CC } do
          if compiler and compiler ~= '' then
            ...
            local compiler_path = vim.fn.exepath(compiler_bin)
        ...
        if vim.env.OECORE_NATIVE_SYSROOT then
          table.insert(query_drivers, vim.env.OECORE_NATIVE_SYSROOT .. '/usr/bin/*/*')
        end

        if vim.env.OECORE_TARGET_SYSROOT then
          table.insert(query_drivers, vim.env.OECORE_TARGET_SYSROOT .. '/../x86_64*/usr/bin/*/*')
        end

        -- Fallback for populate_sdk installs that live outside the env vars above.
